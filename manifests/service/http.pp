        ...
        if $ssl_mode {
                        if $use_auth {
                                nagios::type::service{"https_${name}_${check_string}":
                                        ...
                                        check_command => "check_https_auth_string!${check_domain}!${check_url}!'${check_string}'!${auth_name}!${auth_password}",
                                }
                        }
                        else {
                                nagios::type::service{"https_${name}_${check_string}":
                        ...
                        if $use_auth {
                                nagios::type::service{"http_${name}_${check_string}":
                                        ...
                                        server_name => $server_name,
                                        check_command => "check_http_auth_string!${check_domain}!${check_url}!'${check_string}'!${auth_name}!${auth_password}",
                                }
                        }
                        else {
                                nagios::type::service{"http_${name}_${check_string}":
